...
class CIAB_Admin {
	...
	public function catch_form_submission() {
		...
		elseif ( ! empty( $_REQUEST['cbox-action'] ) && $_REQUEST['cbox-action'] == 'upgrade' ) {
			...
			// set reference pointer for later user
			cbox()->setup = 'upgrade';
	...
	private function instructions() {
		...
		echo '<h2>' . __( 'Instructions', 'cbox' ) . '</h2>';
	...
	<?php
		...
		// workflow 2: cbox hasn't been setup, but buddypress is active
			...
			// doing a function_exists check here because bp_get_maintenance_mode() only exists in BP 1.6+
			// @todo make this BP 1.5-compatible?
			...
			<div class="welcome-panel-column-container">
				...
				<div class="welcome-panel-column welcome-panel-last">
					...
					<?php
						...
						$is_bp_compatible = in_array( 'buddypress', $theme_tags );
						...
						if ( $theme->errors() ) :
							...
							if ( $theme->get_stylesheet() != 'cbox-theme' ) {
								echo '<p>';
								printf( __( 'Your current theme is %s.', 'cbox' ), '<strong>' . $theme->display( 'Name' ) . '</strong>' );
								echo '</p>';

								if ( ! $is_bp_compatible ) {
									echo '<p>';
									printf( __( "It looks like this theme is not compatible with BuddyPress. You'll need to <a href='%s'>select a BuddyPress-compatible theme</a> for CBox to work.", 'buddypress' ), network_admin_url( 'theme-install.php?type=tag&s=buddypress&tab=search' ) );
									echo '</p>';
								}

								echo '<p>';
								printf( __( 'Did you know that CBox comes with a cool theme?  <a href="%s">Check it out here</a>.', 'cbox' ), esc_url( network_admin_url( 'theme-install.php?tab=search&s=%22commons+in+a+box%22' ) ) );
								echo '</p>';
								...
								echo '<p>';
								printf( __( "You're using the CBox Default theme! Good on ya! <a href='%s'>Configure the theme here</a>.", 'cbox' ), esc_url( admin_url( 'themes.php?page=infinity-theme' ) ) );
								echo '</p>';
							}
